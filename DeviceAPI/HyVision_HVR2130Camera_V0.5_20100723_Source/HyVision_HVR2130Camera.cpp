#include "HyVision_HVR2130Camera.h"
...
HyVision_HVR2130Camera::HyVision_HVR2130Camera(void)
...
int HyVision_HVR2130Camera::Initialize(Property parameter)
{
	if( SetParameter(parameter) == API_ERROR ) {
		return API_ERROR;
	}
	PrintMessage("SUCCESS : HyVision_HVR2130Camera::Initialize()\n");
	return API_SUCCESS;
}

int HyVision_HVR2130Camera::Enable(void)
{
	if(! cameraInit(_cameraWidth, _cameraHeight) ) {
		PrintMessage("ERROR : HyVision_HVR2130Camera::Enable() -> Can't init camera");
		return API_ERROR;
	}
	...
	PrintMessage("SUCCESS : HyVision_HVR2130Camera::Enable()\n");
	return API_SUCCESS;
}

int HyVision_HVR2130Camera::Disable(void)
{
	if( _cameraBufBMP ) {
...
int HyVision_HVR2130Camera::Finalize(void)
{
	return API_SUCCESS;
}

int HyVision_HVR2130Camera::SetParameter(Property parameter)
{
	...
	{
		if( !parameter.FindName("CameraID") )
			PrintMessage("Error:HyVision_HVR2130Camera::SetParameter()->Can't find parameter CameraID.\n");
		if( !parameter.FindName("Width") )
			PrintMessage("Error:HyVision_HVR2130Camera::SetParameter()->Can't find parameter Width.\n");
		if( !parameter.FindName("Height") )
			PrintMessage("Error:HyVision_HVR2130Camera::SetParameter()->Can't find parameter Height.\n");
		if( !parameter.FindName("PixelBytes") )
			PrintMessage("Error:HyVision_HVR2130Camera::SetParameter()->Can't find parameter PixelBytes.\n");
		if( !parameter.FindName("Flip") )
			PrintMessage("Error:HyVision_HVR2130Camera::SetParameter()->Can't find parameter Flip.\n");

		PrintMessage("Error:HyVision_HVR2130Camera::SetParameter()->Can't find parameter(s) name.\n");
		return API_ERROR;
	}
	...
	PrintMessage("SUCCESS:HyVision_HVR2130Camera::SetParameter()->CameraID = %d, Width = %d, Height = %d.\n", _id, _width, _height);
...
int HyVision_HVR2130Camera::GetParameter(Property &parameter)
{
...
int HyVision_HVR2130Camera::GetImage(unsigned char *image)
{
	if(1 != HVR_camGetImageData(_cameraWidth*_cameraHeight*2, _cameraBuf, _id))
	{
		PrintMessage("ERROR : HyVision_HVR2130Camera::GetImage() -> Buffer fail...\r\n");
		return API_ERROR;
	}
...
bool HyVision_HVR2130Camera::cameraInit(int width, int height)
{
	int res = VGA;
...
OprosApi *GetAPI()
{
	return new HyVision_HVR2130Camera();
}
