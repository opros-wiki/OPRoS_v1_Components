...
***************************************************************/
#include "Softing_CAN.h"
#include "OprosPrintMessage.h"
#include "SoftingCanImpl.h"

using namespace std;

class SoftingCANInfo
{
public:
	SoftingCANInfo(void)
	{
...
static map<int, SoftingCANInfo> canInfo;

Softing_CAN::Softing_CAN(void)
{
	_channel = -1;
...
int Softing_CAN::Initialize(Property parameter)
{
...
int Softing_CAN::SetParameter(Property parameter)
{
	//	채널 정보 설정
	if(parameter.FindName("Channel") == false)			goto ERROR_Softing_CAN_SetParameter;
	_channel = atoi(parameter.GetValue("Channel").c_str());
	if(_channel < 0)									goto ERROR_Softing_CAN_SetParameter;
	...
	//	채널에 대한 정보가 없으면 설정
	//	Time Out 설정
	if(parameter.FindName("TimeOut") == false)			goto ERROR_Softing_CAN_SetParameter;
	canInfo[_channel].timeOut = (unsigned long)atol(parameter.GetValue("TimeOut").c_str());

	if(parameter.FindName("BitRate") == false)			goto ERROR_Softing_CAN_SetParameter;	
	canInfo[_channel].bitRate = atol(parameter.GetValue("BitRate").c_str());

	if(parameter.FindName("AcceptanceMask") == false)	goto ERROR_Softing_CAN_SetParameter;	
	canInfo[_channel].acceptanceMask = atol(parameter.GetValue("AcceptanceMask").c_str());

	if(parameter.FindName("AcceptanceCode") == false)	goto ERROR_Softing_CAN_SetParameter;	
	canInfo[_channel].acceptanceCode = atol(parameter.GetValue("AcceptanceCode").c_str());

	if(parameter.FindName("Mode") == false)				goto ERROR_Softing_CAN_SetParameter;	
	canInfo[_channel].mode = atoi(parameter.GetValue("Mode").c_str());

	canInfo[_channel].can = new SoftingCanImpl(_channel, canInfo[_channel].bitRate,
...
ERROR_Softing_CAN_SetParameter :
	_channel = -1;
	PrintMessage("ERROR : Softing_CAN::SetParameter -> Can't find a parameter name\n");
	return API_ERROR;
}

int Softing_CAN::GetParameter(Property &parameter)
{
	if(canInfo.find(_channel) == canInfo.end()) {
		return API_ERROR;
	}
...
int Softing_CAN::Write(long id, unsigned char *data, int size)
{
	if(canInfo.find(_channel) == canInfo.end()) {
		return API_ERROR;
	}
...
int Softing_CAN::Read(long &id, unsigned char *data, int size)
{
	if(canInfo.find(_channel) == canInfo.end()) {
		return API_ERROR;
	}
