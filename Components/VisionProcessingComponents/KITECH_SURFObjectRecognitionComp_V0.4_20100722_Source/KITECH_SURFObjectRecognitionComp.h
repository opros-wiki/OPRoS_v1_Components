#ifndef _KITECH_SURFObjectRecognitionComponent_COMPONENT_H
#define _KITECH_SURFObjectRecognitionComponent_COMPONENT_H
/*
...
/**
 *	<table><tr><td>
 *	KITECH_SURFObjectRecognitionComp는 영상을 입력받아 물체를 인식하는 컴포넌트이다.
 *	KITECH_SURFObjectRecognitionComp에는 KITECH_SURFObjectRecognitionComp.xml,VisionRecognitionServiceProvided.h가
 *	포함되어야 하며, KITECH_SURFObjectRecognitionComp.xml는 KITECH_SURFObjectRecognitionComp의 프로파일이 저장되어 있다.
 ...
 */
class KITECH_SURFObjectRecognitionComp : public Component, public IVisionRecognitionService
{
protected:
	// 속성
	//	Variables for KITECH_SURFObjectRecognitionComp
	/**
	 *	KITECH_SURFObjectRecognitionComp의 프로파일정보를 저장하는 변수이다.<br>
	 *	<!-- [표] Parameter -->
	 *	<table border=1>
	 ...
	 *	<tr><td><b>dbName</b></td> <td>std::string</td> <td>컴포넌트에서 사용할 DataBase파일로서 인식할 이미지 파일 이름이다.</td></tr>
	 *	</table>
	*/
	Property parameter;
...
public:
	/**
	 *	KITECH_SURFObjectRecognitionComp 클래스의 기본 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	*/
	KITECH_SURFObjectRecognitionComp();

	/**
	 *	KITECH_SURFObjectRecognitionComp 클래스의 전달인자를 포함한 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	 *	@param compId 컴포넌트의 ID.
	*/
	KITECH_SURFObjectRecognitionComp(const std::string &compId);

	/**
	 *	KITECH_SURFObjectRecognitionComp 클래스의 기본 소멸자이다.
	*/
	virtual ~KITECH_SURFObjectRecognitionComp();

	/**
	 *	KITECH_SURFObjectRecognitionComp에서 제공하는 서비스 포트를 초기화하고 등록한다.
	*/
...
public:
	...
	// 서비스포트
	/**
	 *	KITECH_SURFObjectRecognitionComp의 프로파일을 변경한다.
	...
	/**
	 *	KITECH_SURFObjectRecognitionComp의 프로파일을 다른 컴포넌트에서 가져갈 때 사용된다.
