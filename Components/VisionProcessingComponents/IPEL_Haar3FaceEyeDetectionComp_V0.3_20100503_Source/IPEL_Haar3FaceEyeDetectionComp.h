#ifndef _IPEL_Haar3FaceEyeDetectionComponent_COMPONENT_H
#define _IPEL_Haar3FaceEyeDetectionComponent_COMPONENT_H
/*
...
/**
 *	<table><tr><td>
 *	IPEL_Haar3FaceEyeDetectionComp는 영상을 입력받아 얼굴 및 눈을 인식하는 컴포넌트이다.
 *	색상에 대한 데이터베이스를 구하여 카메라를 통해 받은 영상에서 색상을 판별해낸다.
 *	IPEL_Haar3FaceEyeDetectionComp에는 IPEL_Haar3FaceEyeDetectionComp.xml,FaceRecognitionServicePortProvided.h가
 *	포함되어야 하며, IPEL_Haar3FaceEyeDetectionComp.xml는 IPEL_Haar3FaceEyeDetectionComp의 프로파일이 저장되어 있다.
 *	FaceRecognitionServicePortProvided.h는 이 컴포넌트에서 제공하는 서비스 포트를 호출할 수 있는
 *	인터페이스를 제공한다.
 *	</td></tr><tr><td>
 ...
 * 이 프로젝트 디렉토리에 있는 haarcascade_frontalface_alt.xml eyeL.xml eyeR.xml 파일을 실행파일이 있는 폴더에 두어야 한다.
*/
class IPEL_Haar3FaceEyeDetectionComp : public Component, public IVisionRecognitionService
{
protected:
	// 속성
	//	Variables for IPEL_Haar3FaceEyeDetectionComp
	/**
	 *	IPEL_Haar3FaceEyeDetectionComp의 프로파일정보를 저장하는 변수이다.<br>
	*/
	Property parameter;
	...
	/**
	* 얼굴 위치 검출 클래스이다.
	* 얼굴의 위치 뿐만 아니라 눈의 위치도 같이 검출을 한다. 클래스내에 3개의 
	* 얼굴과 두 눈의 위치를 검출하기 위한 classifier를 따로 갖고 있다.
	*/
	FaceLocator *_faceLocator;

public:
	/**
	 *	IPEL_Haar3FaceEyeDetectionComp 클래스의 기본 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	*/
	IPEL_Haar3FaceEyeDetectionComp();

	/**
	 *	IPEL_Haar3FaceEyeDetectionComp 클래스의 전달인자를 포함한 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	 *	@param compId 컴포넌트의 ID.
	*/
	IPEL_Haar3FaceEyeDetectionComp(const std::string &compId);

	/**
	 *	IPEL_Haar3FaceEyeDetectionComp 클래스의 기본 소멸자이다.
	*/
	virtual ~IPEL_Haar3FaceEyeDetectionComp();

	/**
	 *	IPEL_Haar3FaceEyeDetectionComp에서 제공하는 서비스 포트를 초기화하고 등록한다.
	*/
...
public:
	...
	// 서비스포트
	/**
	 *	IPEL_Haar3FaceEyeDetectionComp의 프로파일을 변경한다.
	...
	/**
	 *	IPEL_Haar3FaceEyeDetectionComp의 프로파일을 다른 컴포넌트에서 가져갈 때 사용된다.
