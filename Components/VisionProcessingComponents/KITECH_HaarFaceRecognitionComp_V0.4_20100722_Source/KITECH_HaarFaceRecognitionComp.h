#ifndef _KITECH_HaarFaceRecognitionComponent_COMPONENT_H
#define _KITECH_HaarFaceRecognitionComponent_COMPONENT_H
/*
...
/**
 *	<table><tr><td>
 *	KITECH_HaarFaceRecognitionComp는 영상을 입력받아 얼굴을 검출하는 컴포넌트이다.
 *	KITECH_HaarFaceRecognitionComp에는 KITECH_HaarFaceRecognitionComp.xml,VisionRecognitionServiceProvided.h가
 *	포함되어야 하며, KITECH_HaarFaceRecognitionComp.xml는 KITECH_HaarFaceRecognitionComp의 프로파일이 저장되어 있다.
 ...
 * 이 프로젝트 디렉토리에 있는 haarcascade_frontalface_alt2.xml 파일을 실행파일이 있는 폴더에 두어야 한다.
 */
class KITECH_HaarFaceRecognitionComp : public Component, public IVisionRecognitionService
{
protected:
	// 속성
	//	Variables for KITECH_HaarFaceRecognitionComp
	/**
	 *	KITECH_HaarFaceRecognitionComp의 프로파일정보를 저장하는 변수이다.<br>
	*/
	Property parameter;
...
public:
	/**
	 *	KITECH_HaarFaceRecognitionComp 클래스의 기본 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	*/
	KITECH_HaarFaceRecognitionComp();

	/**
	 *	KITECH_HaarFaceRecognitionComp 클래스의 전달인자를 포함한 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	 *	@param compId 컴포넌트의 ID.
	*/
	KITECH_HaarFaceRecognitionComp(const std::string &compId);

	/**
	 *	KITECH_HaarFaceRecognitionComp 클래스의 기본 소멸자이다.
	*/
	virtual ~KITECH_HaarFaceRecognitionComp();

	/**
	 *	KITECH_HaarFaceRecognitionComp에서 제공하는 서비스 포트를 초기화하고 등록한다.
	*/
...
public:
	...
	// 서비스포트
	/**
	 *	KITECH_HaarFaceRecognitionComp의 프로파일을 변경한다.
	...
	/**
	 *	KITECH_HaarFaceRecognitionComp의 프로파일을 다른 컴포넌트에서 가져갈 때 사용된다.
