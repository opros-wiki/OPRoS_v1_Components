#ifndef _KITECH_SkinColorFaceRecognitionComponent_COMPONENT_H
#define _KITECH_SkinColorFaceRecognitionComponent_COMPONENT_H
/*
...
/**
 *	<table><tr><td>
 *	KITECH_SkinColorFaceRecognitionComp는 영상을 입력받아 얼굴을 식별하는 컴포넌트이다.
 *	색상에 대한 데이터베이스를 구하여 카메라를 통해 받은 영상에서 색상을 판별해낸다.
 *	KITECH_SkinColorFaceRecognitionComp에는 KITECH_SkinColorFaceRecognitionComp.xml,VisionRecognitionServiceProvided.h가
 *	포함되어야 하며, KITECH_SkinColorFaceRecognitionComp.xml는 KITECH_SkinColorFaceRecognitionComp의 프로파일이 저장되어 있다.
...
*/
class KITECH_SkinColorFaceRecognitionComp : public Component, public IVisionRecognitionService
{
protected:
	// 속성
	//	Variables for KITECH_SkinColorFaceRecognitionComp
	/**
	 *	KITECH_SkinColorFaceRecognitionComp의 프로파일정보를 저장하는 변수이다.<br>
	*/
	Property parameter;
...
public:
	/**
	 *	KITECH_SkinColorFaceRecognitionComp 클래스의 기본 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	*/
	KITECH_SkinColorFaceRecognitionComp();

	/**
	 *	KITECH_SkinColorFaceRecognitionComp 클래스의 전달인자를 포함한 생성자이다.<br>
	 *	포트를 초기화시키는 함수를 호출한다.
	 *	@param compId 컴포넌트의 ID.
	*/
	KITECH_SkinColorFaceRecognitionComp(const std::string &compId);

	/**
	 *	KITECH_SkinColorFaceRecognitionComp 클래스의 기본 소멸자이다.
	*/
	virtual ~KITECH_SkinColorFaceRecognitionComp();

	/**
	 *	KITECH_SkinColorFaceRecognitionComp에서 제공하는 서비스 포트를 초기화하고 등록한다.
	*/
...
public:
	...
	// 서비스포트
	/**
	 *	KITECH_SkinColorFaceRecognitionComp의 프로파일을 변경한다.
	...
	/**
	 *	KITECH_SkinColorFaceRecognitionComp의 프로파일을 다른 컴포넌트에서 가져갈 때 사용된다.
