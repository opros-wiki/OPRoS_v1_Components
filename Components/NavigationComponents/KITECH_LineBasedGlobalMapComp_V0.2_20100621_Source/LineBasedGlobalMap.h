...
class LineBasedGlobalMap : public GridMap<unsigned char> 
{
private:
	unsigned char _palette[256][4];
	vector<MM_RECT> line_items;
...
public:
	LineBasedGlobalMap(int width, int height, double _cellWidth);
	LineBasedGlobalMap(const char *fileName, double _cellWidth);
	~LineBasedGlobalMap ();

	bool LoadDxfMap (const char *fileName, double cellWidth);
};
