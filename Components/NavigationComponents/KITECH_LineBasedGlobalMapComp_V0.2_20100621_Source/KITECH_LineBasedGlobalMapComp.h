#ifndef _KITECH_LineBasedGlobalMapComp_COMPONENT_H
#define _KITECH_LineBasedGlobalMapComp_COMPONENT_H
/*
...
// 개요
/**
*	KITECH_LineBasedGlobalMapComp는 이동 로봇의 주행을 위한 전역맵을 제어하는 컴포넌트이다. 
*	KITECH_LineBasedGlobalMapComp는 격자 형태로 구성되는 전역맵을 제어하는 컴포넌트로써
*	전역맵을 제어하기 위한 표준 인터페이스인 IGlobalMapCompService를 상속받아 구현된다.
*	동적 라이브러리로 생성되며 NavigationComponent에서 KITECH_LineBasedGlobalMapComp의 서비스 함수를 호출하여 사옹한다.
*	KITECH_LineBasedGlobalMapComp는 로드맵 방식의 전역맵 컴포넌트로써 사전에 재작된 전역맵 정보를 가지고 있는 
*	선으로 이루어져 있는 캐드의 DXF 파일을 읽어 사용하기 때문에
*	컴포넌트가 실행시 적재될 전역맵 DXF파일 이름과 적재될 전역맵의 한 픽셀당 가지는 실제 거리 정보와 
*	전역맵을 로봇 안전 반경만큼 확장시키기 위한 이동 로봇의 안전 반경을 알아야 한다.
*/
class KITECH_LineBasedGlobalMapComp: public Component, public IGlobalMapService
{
public:
	KITECH_LineBasedGlobalMapComp();
	KITECH_LineBasedGlobalMapComp(const std::string &compId);
	virtual ~KITECH_LineBasedGlobalMapComp();
...
public:
	...
	*	KITECH_LineBasedGlobalMapComp에서는 사용하지 않는다.
